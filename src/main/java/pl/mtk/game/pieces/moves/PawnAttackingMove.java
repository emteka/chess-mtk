...
@Getter
@ToString
public class PawnAttackingMove extends Move {

    public PawnAttackingMove(Tile.Position start, Tile.Position destination) {
        super(start, destination);
    }

    public PawnAttackingMove() {
        super();
    }
    ...
    public boolean isValid(Chessboard chessboard) {
        ...
        if (isWithinBounds) {
            ...
            if (destinationContainsPiece) {
                Color pieceColor = chessboard.getTile(this.origin).getPiece().getColor();
                boolean destinationContainsPieceOfDifferentColor = !chessboard.getTile(this.destination).getPiece().getColor().equals(pieceColor);
                return destinationContainsPieceOfDifferentColor;
            }
        }
        return false;
    }
